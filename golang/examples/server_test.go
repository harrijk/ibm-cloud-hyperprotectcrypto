...
const (
	address      = "ep11.us-south.hs-crypto.test.cloud.ibm.com:9333"
	apiKey       = "Hw90yHh6HGAqUm19j7hrvNzB_wtM7S9vJQ1wayp9D8w6"
	authEndpoint = "https://iam.test.cloud.ibm.com"
	instanceID   = "367db53c-24cc-4410-962b-3a308fdeaf02"
)

// Example_getMechnismInfo gets mechnism list and retrieve detail information for CKM_RSA_PKCS
func Example_getMechnismInfo() {
	access, err := util.GetToken(apiKey, authEndpoint)
	if err != nil {
		fmt.Printf("%s", err)
	}

	auth := util.IAMPerRPCCredentials{
		Auth:     fmt.Sprintf("Bearer %s", access.Token),
		Instance: instanceID,
	}
...
// Example_getToken obtains a bearer token from the IBM Cloud
func Example_getToken() {
	_, err := util.GetToken(apiKey, authEndpoint)
	if err != nil {
		fmt.Printf("%s", err)
	}

	fmt.Println("Obtained access_token")
