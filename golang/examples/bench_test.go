...
func getMechanismInfo() {
	...
	if err != nil {
		panic(fmt.Errorf("Get mechanism list, response: %s,  error: %s", mechanismListResponse, err))
	}
...
func BenchmarkMechanismInfo(b *testing.B) {
	for i := 0; i < b.N; i++ {
		getMechanismInfo()
	}
}
...
func encryptAndDecryptWorker(id int, wg *sync.WaitGroup) {
	encryptAndDecrypt()
	wg.Done()
}

func EncryptAndDecryptParallel(worker int) {
	var wg sync.WaitGroup
	for j := 0; j < worker; j++ {
		wg.Add(1)
		go encryptAndDecryptWorker(j, &wg)
	}
	wg.Wait()
}
...
func BenchmarkEncryptAndDecryptParallel(b *testing.B) {
	...
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				EncryptAndDecryptParallel(bm.workers)
			}
		})
...
func BenchmarkWrapAndUnwrapKeyParallel(b *testing.B) {
	benchmarks := []struct {
		name    string
		workers int
	}{
...
func digest() {
	...
	if err != nil {
		panic(fmt.Errorf("Digest, response: %s, error: %s", digestResponse, err))
	} else {
	...
	if err != nil {
		panic(fmt.Errorf("Digest final, response: %s, error: %s", digestFinalResponse, err))
	} else {
...
func BenchmarkSignAndVerifyUsingRSAKeyPair(b *testing.B) {
	for i := 0; i < b.N; i++ {
...
func BenchmarkSignAndVerifyUsingECDSAKeyPair(b *testing.B) {
	for i := 0; i < b.N; i++ {
		signAndVerifyUsingECDSAKeyPair()
	}
}

///////////// Sign and Verify with Error Testing /////////////
...
func BenchmarkSignAndVerifyToTestErrorHandling(b *testing.B) {
	for i := 0; i < b.N; i++ {
		signAndVerifyToTestErrorHandling()
	}
}
